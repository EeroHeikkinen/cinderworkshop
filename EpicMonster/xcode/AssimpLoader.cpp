...
void AssimpLoader::setNodePosition( const string &name, const Vec3f &pos )
{
   AssimpNodeRef node = getAssimpNode( name );
   if ( node )
...
Vec3f AssimpLoader::getNodePosition( const string &name )
   {
        AssimpNodeRef node = getAssimpNode( name );
        if ( node )
...
void AssimpLoader::draw()
{
    ...
    unsigned int AssimpLoader::totalTriangles()
    {
        ...
        for ( ; it != mMeshNodes.end(); ++it )
        {
            ...
            for ( ; meshIt != nodeRef->mMeshes.end(); ++meshIt )
            {
                ...
                triangles += assimpMeshRef->mCachedTriMesh.getNumTriangles();
            }
        }
